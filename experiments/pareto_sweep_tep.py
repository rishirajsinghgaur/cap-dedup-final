#!/usr/bin/env python3
"""
Pareto Frontier Sweep on TEP — Phase 1 Q1-readiness deliverable.

Sweeps the three CAP-Dedup gate thresholds:
  tau_low  in {0.05, 0.10, 0.15, 0.20, 0.30, 0.40}   - Level 1 entry cap
  tau_high in {0.15, 0.20, 0.30, 0.50}               - Level 2 preserve cap
  theta    in {0.75, 0.80, 0.85, 0.90, 0.95}         - Level 1 cosine similarity cap

For each (tau_low, tau_high, theta) combo across N seeds, we compute fault recall
and storage savings on the TEP test set and dump every point to a CSV.

The expensive work (data load, framework training, causal discovery, uncertainty
scoring) happens ONCE per seed. Threshold sweeps reuse the trained framework,
so the marginal cost per (tau_low, tau_high, theta) point is just one
find_duplicates_multi_level call.

Knee analysis:
  - For each seed: linearly interpolate to find max storage savings at >=90, 95, 99% recall.
  - Aggregate across seeds (mean ± std).
  - Save to results/pareto/knee_report.json.

USAGE:
  python experiments/pareto_sweep_tep.py                # full sweep, 10 seeds
  python experiments/pareto_sweep_tep.py --quick        # 2 seeds, small grid (~5 min smoke test)
  python experiments/pareto_sweep_tep.py --seeds 30     # custom seed count
"""
...
def train_for_seed(seed, sample_size, config, label_type="gt_faults",
                   split_mode: str = "stratified"):
    """Load TEP data, split (4-way: train/val/cal/test) with stratification or
    episode-level, train framework, run causal discovery, compute uncertainty.

    split_mode:
        'stratified' (DEFAULT, recommended): stratified random split by anomaly label,
            preserving anomaly rate across all splits. Reduces test set composition
            variance. Acceptable for per-sample classification (no temporal model).
        'episode': split by simulationRun (TEP) / file_id (SKAB, SWaT). Stricter
            temporal independence but suffers high variance with uneven episode sizes.

    Returns artefacts dict including SEPARATE 'cal' set (used ONLY for conformal
    calibration, never for BNN training/early-stopping).
    """
    logger.info(f"[seed={seed}] loading TEP data ({sample_size} samples, {label_type}, split_mode={split_mode})")
    set_seeds(seed)

    loader = TEPDataLoader()
    df, safety_labels, _duplicate_pairs, raw_df = loader.load_data(
        sample_size=sample_size, random_state=seed, label_type=label_type
    )
    safety_labels = safety_labels.astype(int)

    cfg = config.copy()
    cfg["model"] = config["model"].copy()
    cfg["model"]["input_dim"] = 52  # TEP
    # Allow the BNN ablation script to override architecture knobs via
    # environment variables .
    import os
    if os.environ.get("CAPDEDUP_BNN_NUM_MODELS"):
        cfg["model"]["num_ensemble_models"] = int(os.environ["CAPDEDUP_BNN_NUM_MODELS"])
    if os.environ.get("CAPDEDUP_BNN_DROPOUT"):
        cfg["model"]["bayesian_dropout_rate"] = float(os.environ["CAPDEDUP_BNN_DROPOUT"])
    logger.info(f"[seed={seed}] BNN config: num_models={cfg['model']['num_ensemble_models']}, "
                f"dropout_rate={cfg['model'].get('bayesian_dropout_rate', 0.3)}")

    framework = UncertaintyAwareFramework(cfg)

    # 4-way split: train / val (BNN early-stop) / cal (conformal calibration) / test
    ...
    # Train Bayesian + Siamese on TRAIN; val for early stopping only
    ...
    return dict(
        ...
        y_test=y_test,
        u_test=u_test,
        # cal set is the HELD-OUT conformal calibration data (never seen by BNN).
        # val set is used only for BNN early-stopping (legacy compat; not used by
        # conformal gate after this refactor).
        X_cal=X_cal,
        y_cal=y_cal,
        X_val=X_val,
        y_val=y_val,
...
def run_sweep(args):
    ...
    if args.quick:
        ...
        theta_grid = [0.85]
        conformal_recall_grid = [None, 0.90, 0.95, 0.99]
        coreset_budget_grid = [None, 0.8, 0.6, 0.4, 0.3, 0.2]
        priority_fraction_grid = [0.15, 0.20, 0.25, 0.30, 0.40, 0.50]
        budget_fraction_grid = [0.25, 0.40, 0.55, 0.70, 0.85]
    ...
    csv_path = out_dir / "pareto_sweep_tep.csv"
    ...
    for seed in seeds:
        ...
        X_train = artefacts.get("X_train")
        y_train = artefacts.get("y_train")
        # NEW: separate calibration set (NOT used for BNN training/early-stopping)
        X_cal = artefacts["X_cal"]
        y_cal = artefacts["y_cal"]
        n_test = len(df_test)

        # ---- Conformal Layer 0 (Path B+) — multi-scorer setup ----
        # Build scorer roster. Score on the HELD-OUT calibration set + test set.
        ...
        # Fit conformal gates per (scorer, target_recall) using the CAL set
        # (independent of training/early-stopping → exchangeability holds).
        conformal_masks = {}
        ...
        # ====================================================================
        # Literature baselines (Random / Reservoir / Stratified-by-score /
        # k-center / Facility-location) at MATCHED budgets. These isolate the
        # contribution of CAP-Dedup's conformal+priority architecture.
        # ====================================================================
        n_baseline_rows = 0
        # Use ECOD scorer for the "anomaly score" the baselines need (consistent
        # across datasets per the paper's recommendation; deterministic on each test set).
        if scored_cache.get("ecod") is not None:
            ecod_test_scores = scored_cache["ecod"][1]
        else:
            ecod_test_scores = u_test  # fallback
        for budget_frac in budget_fraction_grid:
            ...
            for bname, keep_mask in baseline_masks.items():
                ...
                rows.append(dict(
                    ...
                    coreset_budget=budget_frac,
                    safety_recall=float(rec),
                    storage_savings_pct=float(sav),
                    duplicate_pairs=0,
                    samples_removed=int(n_test - keep_mask.sum()),
                    n_test=n_test, level0_count=0,
                    level1_count=0, level2_count=0, level3_count=0,
    ...
    try:
        ...
        ax.set_title(f"CAP-Dedup + Conformal Layer 0: Pareto Frontier on TEP "
                     f"({args.sample_size} samples, {df_rows['seed'].nunique()} seeds, "
        ...
        plot_path = out_dir / "pareto_frontier_tep.png"
        fig.tight_layout()
    ...
    # ------------------------------------------------------------------------
    print("\n" + "=" * 78)
    print("PARETO SWEEP - KNEE REPORT (TEP, label_type=" + args.label_type + ")")
    print("=" * 78)
    ...
    print(f"Plot:  {out_dir / 'pareto_frontier_tep.png'}")
    print("=" * 78)
...
def main():
    ...
    p.add_argument("--seed-start", type=int, default=42,
                   help="first modelling seed (default 42); seeds run from "
                        "seed_start to seed_start+seeds-1")
    p.add_argument("--sample-size", type=int, default=10000,
                   help="TEP sample size (default 10000, matches paper)")
    p.add_argument("--label-type", default="gt_faults",
                   choices=["gt_faults", "gt_subset", "proxy"],
                   help="label scheme for fault recall (default gt_faults)")
    p.add_argument("--split-mode", default="stratified",
                   choices=["stratified", "episode"],
                   help="Data split mode: 'stratified' (recommended, low variance) or "
                        "'episode' (strict temporal isolation, higher variance)")
    args = p.parse_args()
    run_sweep(args)
