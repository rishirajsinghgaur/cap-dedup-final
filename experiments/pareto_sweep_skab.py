#!/usr/bin/env python3
"""
Pareto Frontier Sweep on SKAB (Phase 2 multi-dataset validation).

Same CAP-Dedup framework as pareto_sweep_tep.py applied to the Skoltech
Anomaly Benchmark:
  - 8 sensor features (vs TEP's 52)
  - Binary anomaly labels (ground truth)
  - ~47k rows total across 35 files
  - 28% anomaly rate

Splits by file_id (analogous to TEP's simulationRun) so train/test respects
the natural episode boundaries.

USAGE:
  python experiments/pareto_sweep_skab.py                # full 10-seed sweep
  python experiments/pareto_sweep_skab.py --quick        # 2-seed smoke test

Output: results/pareto_skab/{pareto_sweep_skab.csv, knee_report.json, *.png}
"""
...
def train_for_seed(seed, sample_size, config, label_type="gt_anomaly",
                   split_mode: str = "stratified"):
    """Load SKAB data, split 4-way (train/val/cal/test) using stratified random
    (default) or episode-level mode. SKAB has 8 features + binary anomaly labels."""
    logger.info(f"[seed={seed}] loading SKAB data (split_mode={split_mode})")
    set_seeds(seed)

    df, safety_labels, raw_df = load_skab(
        sample_size=sample_size if sample_size and sample_size < 47000 else None,
        random_state=seed,
    )
...
def run_sweep(args):
    ...
    out_dir = ROOT / "results" / "pareto_skab"
    out_dir.mkdir(parents=True, exist_ok=True)
    csv_path = out_dir / "pareto_sweep_skab.csv"
    ...
    for seed in seeds:
        ...
        # Score on the HELD-OUT calibration set + test set (val not used here).
    ...
    try:
        ...
        ax.set_title(f"CAP-Dedup: Pareto Frontier on SKAB "
        ...
        plot_path = out_dir / "pareto_frontier_skab.png"
        fig.tight_layout()
    ...
    # ------------------------------------------------------------------------
    print("\n" + "=" * 78)
    print("PARETO SWEEP - KNEE REPORT (SKAB, label_type=" + args.label_type + ")")
    print("=" * 78)
    ...
    print(f"Plot:  {out_dir / 'pareto_frontier_skab.png'}")
    print("=" * 78)
...
def main():
    ...
    p.add_argument("--sample-size", type=int, default=None,
                   help="SKAB sample size (default: use full ~47k rows)")
    p.add_argument("--label-type", default="gt_anomaly",
                   choices=["gt_anomaly"],
                   help="label scheme (only gt_anomaly available for SKAB)")
